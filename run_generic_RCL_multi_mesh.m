...
% Sort mesh
mesh_folder = "/users/PAS1622/richardsui01/electric_sim_gh-1/mesh_data/test_mesh/";

Mesh_file_list = dir(fullfile(mesh_folder,'*.mat')); % Load all the mesh file
Mesh_name_list = {Mesh_file_list.name};
name_idx = zeros(length(Mesh_name_list),1);
All_mesh_partition_num = 0;

for i = 1:length(Mesh_name_list) % Extract number in the file
    tokens = regexp(Mesh_name_list{i},'FEMDATA_(\d+).mat','tokens');
    if ~isempty(tokens)
        name_idx(i) = str2double(tokens{1}{1});
    end
    load(mesh_folder + Mesh_name_list{i});
    All_mesh_partition_num = All_mesh_partition_num + FEM_data.partition_number;
end
[~,sorted_idx] = sort(name_idx);% Sort name
Mesh_name_list = Mesh_name_list(sorted_idx); % Mesh_name_list now in correct order 1,2,3,4...
mesh_num = length(Mesh_name_list);
...
scratchDir = "/fs/scratch/PAS1622/richardsui01/electric_sim_gh-1/test_save_data/";

if ~exist(scratchDir, 'dir')
    mkdir(scratchDir)
    disp('makedir activated')
else
...
scale_gj_loc = gj_chan_vec(1);
scale_chan_loc = gj_chan_vec(2);
...
parfor mesh_idx = 1:mesh_num

    FEM_data = load(mesh_folder + Mesh_name_list{mesh_idx}); 
    FEM_data = FEM_data.FEM_data;
    ...
    save_name = string(mesh_idx) + "_cycle" + string(bcl) + "_beats" + string(nbeats) + "_D" + string(D) ...
              + "_gj_loc" + string(scale_gj_loc) + "_chan_loc" + string(scale_chan_loc);
    % add further mesh names as needed if we have>2 diff IDs 
    if strcmp(tissue,'1D Mdisc cleft ID EpC hetg tissue')
    ...
    %     end
        ...
        if ~mod(ti, dt_samp) && strcmp(tissue,'1D Mdisc cleft ID EpC hetg tissue')
    ...
    tup_axial = tup(1:Nint:end);
    
    cv = 100 * (i2 - i1) * (p.L / 1000) ./ (tup_axial(i2) - tup_axial(i1));
    % disp("Max memory used = " + string(maxMemUsed/1000) + " MB")
    % Cleft idx Data saving
    icleft_all = iEC(1:end-1); 
    idx_pre_all = indices.ind_disc_pre';
    idx_post_all = indices.ind_disc_post';
    
    % Cleft to be saved and its index
    cleft_num = round(Ncell/2); % Middle cleft we want to save
    cleft_idx = icleft_all(1 + (cleft_num - 1)*Mdisc : Mdisc + (cleft_num - 1)*Mdisc);
    idx_pre = idx_pre_all(1 + (cleft_num - 1)*Mdisc : Mdisc + (cleft_num - 1)*Mdisc);
    idx_post = idx_post_all(1 + (cleft_num - 1)*Mdisc : Mdisc + (cleft_num - 1)*Mdisc);
    %save rest of data
    if save_flag_data   
        p.loc_vec = loc_vec;
        ...
        save_cleft_idx(local_save_name,cleft_idx,idx_pre,idx_post)
    end
...
function save_cleft_idx(local_save_name,cleft_idx,idx_pre,idx_post)
    
    save(local_save_name,'cleft_idx','idx_pre','idx_post','-append')
end
